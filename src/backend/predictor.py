...
MODEL_CONFIGS: dict[str, dict[str, Any]] = {
    "bioclip-v1": {
        ...
        "name": "Fast",
        "size_mb": 1400,  # ~600 MB weights + ~800 MB vocab
        # Hosted vocab — create at https://huggingface.co/new-dataset
        # then upload species_embeddings.npz generated by scripts/build_embeddings.py
        "vocab_hf_repo": "PLACEHOLDER/wildlife-id-vocab",
        "vocab_filename": "bioclip-v1/species_embeddings.npz",
        "vocab_size_bytes": 800_000_000,
    },
    "bioclip-v2": {
        ...
        "weight_file": "open_clip_pytorch_model.bin",
        "name": "Accurate",
        "size_mb": 2500,  # ~1700 MB weights + ~800 MB vocab
        "vocab_hf_repo": "PLACEHOLDER/wildlife-id-vocab",
        "vocab_filename": "bioclip-v2/species_embeddings.npz",
        "vocab_size_bytes": 800_000_000,
    },
}
...
class Predictor:
    ...
    def download_model(self, model_id: str) -> Generator[dict[str, Any], None, None]:
        ...
        repo_id = cfg["hf_repo"]
        vocab_repo: str | None = cfg.get("vocab_hf_repo")
        vocab_filename: str | None = cfg.get("vocab_filename")
        vocab_size: int = cfg.get("vocab_size_bytes", 0) if vocab_repo else 0
        ...
        total_bytes = model_bytes + vocab_size
        ...
        def _do_download() -> None:
            try:
                ...
                # Download species embeddings if vocab repo is configured and file not present
                if vocab_repo and vocab_filename:
                    emb_path = model_path / "species_embeddings.npz"
                    if not emb_path.exists():
                        from huggingface_hub import hf_hub_url
                        import requests as _req
                        url = hf_hub_url(repo_id=vocab_repo, filename=vocab_filename, repo_type="dataset")
                        logger.info("Downloading vocab from %s", url)
                        vocab_received = 0
                        ...
                        with _req.get(url, stream=True, timeout=600) as r:
                            ...
                            with open(tmp_path, "wb") as f:
                                for chunk in r.iter_content(chunk_size=1 << 20):
                                    if chunk:
                                        f.write(chunk)
                                        vocab_received += len(chunk)
                                        _emit(model_bytes + vocab_received)
                        tmp_path.rename(emb_path)
                        logger.info("Vocab saved → %s (%.1f MB)", emb_path, emb_path.stat().st_size / 1e6)
